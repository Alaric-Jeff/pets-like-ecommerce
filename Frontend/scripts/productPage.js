...
async function fetchReviews(productId) {
  try {
    const res = await fetch(`http://localhost:3000/get-reviews?productId=${productId}`, { method: "GET" });
    ...
    data.reviews.forEach(function(review) {
      ...
      reviewEl.innerHTML =
        `<p class="reviewer-name">${review.user.firstname} ${review.user.surname}</p>` +
        `<p class="review-text">${review.reviewtext}</p>` +
        `<p class="review-rating">Rating: ${review.rating}/5</p>`;
      if (review.ReviewLikes && review.ReviewLikes.length > 0) {
        reviewEl.innerHTML += `<p class="review-liked">You liked this review</p>`;
      }
